...
use crate::class_loader::{class_file::ClassFile, constant_pool_info::ConstantPoolInfo};

use self::{
    numeric_constant::NumericConstant, string_constant::StringConstant,
    sym_ref_class_or_interface::SymRefClassOrInterface,
    sym_ref_dynamically_computed_call_site::SymRefDynamicallyComputedCallSite,
    sym_ref_dynamically_computed_constant::SymRefDynamicallyComputedConstant,
    sym_ref_field_of_class_or_interface::SymRefFieldOfClassOrInterface,
    sym_ref_method_handle::SymRefMethodHandle, sym_ref_method_of_class::SymRefMethodOfClass,
    sym_ref_method_of_interface::SymRefMethodOfInterface, sym_ref_method_type::SymRefMethodType,
};
...
impl RuntimeConstantPool {
    ...
    pub fn resolve(index: u16, class_file: &ClassFile) -> Option<RuntimeConstants> {
        match class_file.constant_pool.get(index).get_tag() {
            ...
            9 => Some(RuntimeConstants::SymRefFieldOfClassOrInterface(
                SymRefFieldOfClassOrInterface::resolve(index, class_file),
            )),
            10 => Some(RuntimeConstants::SymRefMethodOfClass(
                SymRefMethodOfClass::resolve(index, class_file),
            )),
            11 => Some(RuntimeConstants::SymRefMethodOfInterface(
                SymRefMethodOfInterface::resolve(index, class_file),
            )),
            15 => Some(RuntimeConstants::SymRefMethodHandle(
                SymRefMethodHandle::resolve(index, class_file),
            )),
            16 => Some(RuntimeConstants::SymRefMethodType(
                SymRefMethodType::resolve(index, class_file),
            )),
            17 => Some(RuntimeConstants::SymRefDynamicallyComputedConstant(
                SymRefDynamicallyComputedConstant::resolve(index, class_file),
            )),
            18 => Some(RuntimeConstants::SymRefDynamicallyComputedCallSite(
                SymRefDynamicallyComputedCallSite::resolve(index, class_file),
            )),
            8 => Some(RuntimeConstants::StringConstant(StringConstant::resolve(
                index, class_file,
            ))),
            3..=6 => Some(RuntimeConstants::NumericConstant(NumericConstant::resolve(
                index, class_file,
            ))),
            _ => None,
        }
