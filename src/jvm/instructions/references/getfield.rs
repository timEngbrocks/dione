use crate::{jvm::{frame::Frame, instructions::{Instruction, InstructionResult}, runtime_constant_pool::RuntimeConstants, types::{Types, Value}}, class_loader::parser::{Parser, U2, U1}, opcodes, util::heap::ReferencePtr};
...
impl Instruction for GETFIELD {
	...
	fn execute(&self, execution_context: &mut Frame) -> InstructionResult {
		...
		println!("Getting field `{}.{} - {}` from object `{}`", &field_ref.class_ref.name, &field_ref.name, &field_ref.descriptor, &object.name);
