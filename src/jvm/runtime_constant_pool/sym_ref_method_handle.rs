...
use crate::{class_loader::{constant_pool_info::{ConstantPool, ConstantPoolInfoType, constant_method_handle_info::ConstantMethodHandleKind}, class_file::compare_class_file_version_to_global}, resolve_constant};
...
impl RuntimeConstant for SymRefMethodHandle {
	fn resolve(index: u16, constant_pool: &ConstantPool) -> Self {
		let method_handle = resolve_constant!(ConstantPoolInfoType::MethodHandle, index, constant_pool);
		...
		match kind {
			...
			ConstantMethodHandleKind::PutStatic => {
				...
				let field_ref = SymRefFieldOfClassOrInterface::resolve(method_handle.reference_index, constant_pool);
				SymRefMethodHandle {
			...
			ConstantMethodHandleKind::NewInvokeSpecial => {
				...
				let method_ref = SymRefMethodOfClass::resolve(method_handle.reference_index, constant_pool);
				SymRefMethodHandle {
				...
				if compare_class_file_version_to_global(52, 0) == Ordering::Less {
					...
					match constant_pool.get(index) {
						...
						ConstantPoolInfoType::InterfaceMethodref(_) => {
							let method_ref = SymRefMethodOfInterface::resolve(method_handle.reference_index, constant_pool);
							SymRefMethodHandle {
