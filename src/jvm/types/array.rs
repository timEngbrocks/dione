...
pub trait Array {
    ...
    fn new(kind: Self::Type, length: usize) -> Self
    where
        Self: Sized;
...
impl Array for PrimitiveArray {
    ...
    {
        match kind {
            Types::Reference(_) | Types::ReturnAddress(_) => panic!("Invalid primitive array type"),
            _ => (),
        };
        ...
        Self: Sized,
    {
        ...
        match kind {
            ReferenceArrayKind::Class(ref class_ref, _) => {
                for _ in 0..length {
                    data.push(ReferenceArrayKind::Class(
                        class_ref.clone(),
                        Reference::new(),
                    ));
                }
            ...
            ReferenceArrayKind::Array(ref class_ref, _) => {
                for _ in 0..length {
                    data.push(ReferenceArrayKind::Array(
                        class_ref.clone(),
                        Reference::new(),
                    ));
                }
            ...
            ReferenceArrayKind::Interface(ref class_ref, _) => {
                for _ in 0..length {
                    data.push(ReferenceArrayKind::Interface(
                        class_ref.clone(),
                        Reference::new(),
                    ));
                }
...
impl ReferenceArray {
    pub fn new_preinitialized(kind: ReferenceArrayKind, length: usize) -> Self
    where
        Self: Sized,
    {
        let mut data = Vec::with_capacity(length);
