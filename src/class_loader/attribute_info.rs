use self::{
    attribute_annotation_default::AttributeAnnotationDefault,
    attribute_bootstrap_methods::AttributeBootstrapMethods, attribute_code::AttributeCode,
    attribute_constant_value::AttributeConstantValue, attribute_deprecated::AttributeDeprecated,
    attribute_enclosing_method::AttributeEnclosingMethod,
    attribute_exceptions::AttributeExceptions, attribute_inner_classes::AttributeInnerClasses,
    attribute_line_number_table::AttributeLineNumberTable,
    attribute_local_variable_table::AttributeLocalVariableTable,
    attribute_local_variable_type_table::AttributeLocalVariableTypeTable,
    attribute_method_parameters::AttributeMethodParameters, attribute_nest_host::AttributeNestHost,
    attribute_nest_members::AttributeNestMembers,
    attribute_permitted_subclasses::AttributePermittedSubclasses,
    attribute_record::AttributeRecord,
    attribute_runtime_invisible_annotations::AttributeRuntimeInvisibleAnnotations,
    attribute_runtime_visibile_annotations::AttributeRuntimeVisibleAnnotations,
    attribute_signature::AttributeSignature, attribute_source_file::AttributeSourceFile,
    attribute_stack_map_table::AttributeStackMapTable,
};

use super::{
    constant_pool_info::{ConstantPool, ConstantPoolInfoType},
    parser::Parser,
};
...
impl Attribute for AttributeInfo {
    fn new(parser: &mut Parser, constant_pool: &ConstantPool) -> Self {
        ...
        match attribute_name.as_str() {
            "ConstantValue" => {
                AttributeInfo::ConstantValue(AttributeConstantValue::new(parser, constant_pool))
            }
            "Code" => AttributeInfo::Code(AttributeCode::new(parser, constant_pool)),
            "StackMapTable" => {
                AttributeInfo::StackMapTable(AttributeStackMapTable::new(parser, constant_pool))
            }
            "BootstrapMethods" => AttributeInfo::BootstrapMethods(AttributeBootstrapMethods::new(
                parser,
                constant_pool,
            )),
            "NestHost" => AttributeInfo::NestHost(AttributeNestHost::new(parser, constant_pool)),
            "NestMembers" => {
                AttributeInfo::NestMembers(AttributeNestMembers::new(parser, constant_pool))
            }
            "PermittedSubclasses" => AttributeInfo::PermittedSubclasses(
                AttributePermittedSubclasses::new(parser, constant_pool),
            ),
            "LineNumberTable" => {
                AttributeInfo::LineNumberTable(AttributeLineNumberTable::new(parser, constant_pool))
            }
            "SourceFile" => {
                AttributeInfo::SourceFile(AttributeSourceFile::new(parser, constant_pool))
            }
            "LocalVariableTable" => AttributeInfo::LocalVariableTable(
                AttributeLocalVariableTable::new(parser, constant_pool),
            ),
            "InnerClasses" => {
                AttributeInfo::InnerClasses(AttributeInnerClasses::new(parser, constant_pool))
            }
            "Exceptions" => {
                AttributeInfo::Exceptions(AttributeExceptions::new(parser, constant_pool))
            }
            "RuntimeVisibleAnnotations" => AttributeInfo::RuntimeVisibleAnnotations(
                AttributeRuntimeVisibleAnnotations::new(parser, constant_pool),
            ),
            "LocalVariableTypeTable" => AttributeInfo::LocalVariableTypeTable(
                AttributeLocalVariableTypeTable::new(parser, constant_pool),
            ),
            "Signature" => AttributeInfo::Signature(AttributeSignature::new(parser, constant_pool)),
            "Deprecated" => {
                AttributeInfo::Deprecated(AttributeDeprecated::new(parser, constant_pool))
            }
            "Record" => AttributeInfo::Record(AttributeRecord::new(parser, constant_pool)),
            "MethodParameters" => AttributeInfo::MethodParameters(AttributeMethodParameters::new(
                parser,
                constant_pool,
            )),
            "EnclosingMethod" => {
                AttributeInfo::EnclosingMethod(AttributeEnclosingMethod::new(parser, constant_pool))
            }
            "AnnotationDefault" => AttributeInfo::AnnotationDefault(
                AttributeAnnotationDefault::new(parser, constant_pool),
            ),
            "RuntimeInvisibleAnnotations" => AttributeInfo::RuntimeInvisibleAnnotations(
                AttributeRuntimeInvisibleAnnotations::new(parser, constant_pool),
            ),
            name => {
