...
use attribute_code::{AttributeCode, code_parser};
use attribute_line_number_table::{AttributeLineNumberTable, line_number_table_parser};
use attribute_source_file::{AttributeSourceFile, source_file_parser};
use nom::{
    IResult,
    error::ParseError,
...
pub fn attribute_info_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
    constant_pool: &Vec<CPInfo>,
) -> IResult<&'a [u8], AttributeInfo> {
    let (input, attribute_name_index) =
        be_u16::<&[u8], E>(input).expect("Failed to read 'attribute_name_index'");
    ...
    let (input, _attribute_length) =
        be_u32::<&[u8], E>(input).expect("Failed to read 'attribute_length'");
    match attribute_name.as_str() {
        "Code" => {
            let (input, value) =
                code_parser::<E>(input, constant_pool).expect("Failed to parse 'code'");
            Ok((input, AttributeInfo::Code(value)))
        }
        "LineNumberTable" => {
            let (input, value) = line_number_table_parser::<'a, E>(input)
                .expect("Failed to parse 'line_number_table'");
            Ok((input, AttributeInfo::LineNumberTable(value)))
        }
        "SourceFile" => {
            let (input, value) =
                source_file_parser::<'a, E>(input).expect("Failed to parse 'source_file'");
