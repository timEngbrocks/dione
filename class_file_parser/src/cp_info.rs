...
use class::{Class, class_parser};
use double::{Double, double_parser};
use dynamic::{Dynamic, dynamic_parser};
use fieldref::{Fieldref, fieldref_parser};
use float::{Float, float_parser};
use integer::{Integer, integer_parser};
use interface_methodref::{InterfaceMethodref, interfacemethodref_parser};
use invoke_dynamic::{InvokeDynamic, invokedynamic_parser};
use long::{Long, long_parser};
use method_handle::{MethodHandle, methodhandle_parser};
use method_type::{MethodType, methodtype_parser};
use methodref::{Methodref, methodref_parser};
use module::{Module, module_parser};
use name_and_type::{NameAndType, nameandtype_parser};
use nom::{IResult, error::ParseError, number::complete::be_u8};
use package::{Package, package_parser};
use string::string_parser;
use utf8::{Utf8, utf8_parser};
use util::numbers::U1;
...
pub fn cp_info_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
) -> IResult<&'a [u8], CPInfo> {
    ...
    match tag {
        ...
        FIELDREF => {
            let (input, value) =
                fieldref_parser::<'a, E>(input).expect("Failed to read 'FIELDREF'");
            Ok((input, CPInfo::Fieldref(value)))
        }
        METHODREF => {
            let (input, value) =
                methodref_parser::<'a, E>(input).expect("Failed to read 'METHODREF'");
            Ok((input, CPInfo::Methodref(value)))
        }
        INTERFACEMETHODREF => {
            let (input, value) = interfacemethodref_parser::<'a, E>(input)
                .expect("Failed to read 'INTERFACEMETHODREF'");
        ...
        NAMEANDTYPE => {
            let (input, value) =
                nameandtype_parser::<'a, E>(input).expect("Failed to read 'NAMEANDTYPE'");
        ...
        METHODHANDLE => {
            let (input, value) =
                methodhandle_parser::<'a, E>(input).expect("Failed to read 'METHODHANDLE'");
            Ok((input, CPInfo::MethodHandle(value)))
        }
        METHODTYPE => {
            let (input, value) =
                methodtype_parser::<'a, E>(input).expect("Failed to read 'METHODTYPE'");
        ...
        INVOKEDYNAMIC => {
            let (input, value) =
                invokedynamic_parser::<'a, E>(input).expect("Failed to read 'INVOKEDYNAMIC'");
