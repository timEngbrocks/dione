...
use util::numbers::{U2, U4};

use crate::{
    attribute_info::{AttributeInfo, attribute_info_parser},
    cp_info::{CPInfo, cp_info_parser},
    field_info::{FieldInfo, field_info_parser},
    method_info::{MethodInfo, method_info_parser},
};
...
pub fn class_file_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
    ...
    let (input, constant_pool) =
        length_count(map(be_u16, |i| i - 1), cp_info_parser::<'a, E>)(input)
            .expect("Failed to read 'constant_pool_count' or 'constant_pool'");
    ...
    let (input, interfaces) = length_count(be_u16::<&[u8], E>, be_u16::<&[u8], E>)(input)
        .expect("Failed to read 'interfaces_count' or 'interfaces'");
    let interfaces_count = interfaces.len() as U2;
    let (input, fields) =
        length_count(be_u16, |x| field_info_parser::<E>(x, &constant_pool))(input)
            .expect("Failed to read 'fields_count' or 'fields'");
    let fields_count = fields.len() as U2;
    let (input, methods) =
        length_count(be_u16, |x| method_info_parser::<E>(x, &constant_pool))(input)
            .expect("Failed to read 'methods_count' or 'methods'");
