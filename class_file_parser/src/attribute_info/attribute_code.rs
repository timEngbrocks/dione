...
use util::numbers::{U1, U2, U4};

use crate::cp_info::CPInfo;

use super::{AttributeInfo, attribute_info_parser};
...
impl fmt::Display for AttributeCode {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "Max Stack: {}, Max Locals: {}, Code Length: {}, Code: {}, Exception Table Length: {}, Exception Table: [{}], Attributes Count: {}, Attributes: [{}]",
            self.max_stack,
            self.max_locals,
            self.code_length,
            code,
...
pub fn code_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
    constant_pool: &Vec<CPInfo>,
) -> IResult<&'a [u8], AttributeCode> {
    ...
    let (input, code) = length_count(be_u32, be_u8::<&[u8], E>)(input)
        .expect("Failed to read 'code_length' or 'code'");
    let code_length = code.len() as U4;
    let (input, exception_table) =
        length_count(be_u16, exception_table_entry_parser::<'a, E>)(input)
            .expect("Failed to read 'exception_table_length' or 'exception_table'");
...
pub fn exception_table_entry_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
) -> IResult<&'a [u8], ExceptionTableEntry> {
