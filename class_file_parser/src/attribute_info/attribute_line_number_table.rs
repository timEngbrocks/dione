...
use nom::{IResult, error::ParseError, multi::length_count, number::streaming::be_u16};
use util::numbers::U2;
...
impl fmt::Display for AttributeLineNumberTable {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "Line Number Table Length: {}, Line Number Table: [{}]",
            self.line_number_table_length, line_number_table
        )?;
        Ok(())
    }
}

pub fn line_number_table_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
) -> IResult<&'a [u8], AttributeLineNumberTable> {
    let (input, line_number_table) =
        length_count(be_u16, line_number_table_entry_parser::<'a, E>)(input)
            .expect("Failed to read 'line_number_table_length' or 'line_number_table'");
...
pub fn line_number_table_entry_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
) -> IResult<&'a [u8], LineNumberTableEntry> {
