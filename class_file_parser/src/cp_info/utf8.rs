...
impl fmt::Display for Utf8 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "Tag: {}, Length: {}, Bytes: {}",
            self.tag, self.length, string
        )?;
        Ok(())
    }
}

pub fn utf8_parser<'a, E: ParseError<&'a [u8]> + std::fmt::Debug>(
    input: &'a [u8],
) -> IResult<&'a [u8], Utf8> {
    let (input, bytes) = length_count(be_u16::<&[u8], E>, be_u8::<&[u8], E>)(input)
        .expect("Failed to read 'length' or 'bytes'");
